# -*- coding: utf-8 -*-
"""
Created on Thu Dec 28 10:26:09 2017
...
def weight_variable(shape, name=None, summa=True):
...
def bias_variable(shape, name=None, summa=False):
...
with tf.name_scope("C_P_1"):
    #inception module 1:
    # 1x1 convolutions to reduce dimentionnality a little before the big 3x3 and 5x5 conv layers:
    W_conv1_1x1_1 = weight_variable([1, 1, 3,8], name="W_conv1_1x1_1", summa=False); b_conv1_1x1_1 = bias_variable([8])
    W_conv1_1x1_2 = weight_variable([1, 1, 3,8], name="W_conv1_1x1_2", summa=False); b_conv1_1x1_2 = bias_variable([8])
    W_conv1_1x1_3 = weight_variable([1, 1, 3,8], name="W_conv1_1x1_3", summa=False); b_conv1_1x1_3 = bias_variable([8])
    h_conv1_1x1_1=tf.nn.relu(  conv2d(x_image, W_conv1_1x1_1)  +  b_conv1_1x1_1  ,name="h_conv1_1x1_1")
    h_conv1_1x1_2=tf.nn.relu(  conv2d(x_image, W_conv1_1x1_2)  +  b_conv1_1x1_2  ,name="h_conv1_1x1_2")
    h_conv1_1x1_3=tf.nn.relu(  conv2d(x_image, W_conv1_1x1_3)  +  b_conv1_1x1_3  ,name="h_conv1_1x1_3")
    # a 3x3, a 5x5, a max and an avg pool:
    ...
    W_conv1_1 = weight_variable([2, 2,  3,   8],name="W_conv1_1");b_conv1_1 = bias_variable([8])
    h_conv1_1 = tf.nn.relu(  conv2d(x_image, W_conv1_1)  +  b_conv1_1  ,name="h_conv1_1")
...
summary = see.run(merge,feed_dict={x: np.array(X_train1[22,:]).reshape(-1,224*224*3),y_:np.array(y_train1.iloc[22,:]).reshape(-1,4),
                                   keep_prob: 1})
writer = tf.summary.FileWriter("C:/Users/woill/.spyder-py3/viz/tmp/log", see.graph)
writer.add_summary(summary, i)
...
xxxx=tf.placeholder(tf.float32,name="xxxx")
xxxx_image = tf.reshape(xxxx, [-1,  res_l, res_h,  3])
tf.summary.image(name="im_tes",tensor = xxxx_image,max_outputs=1)
see.run(tf.global_variables_initializer())
merge = tf.summary.merge_all()
summary =see.run(merge,feed_dict = {xxxx :np.array(X.iloc[1,:]).reshape(-1,224*224*3)})
writer.add_summary(summary, i)
...
pix = im_redi.load()
print (im_redi.size) #Get the width and hight of the image for iterating over
...
pixel_values = list(im_redi.getdata())
pixel_values = np.array(pixel_values).reshape((224, 224, 3))
...
xxxx=tf.placeholder(tf.float32,name="xxxx")
xxxx_image = tf.reshape(xxxx, [-1,  res_l, res_h,  3])
tf.summary.image(name="im_tes",tensor = xxxx_image,max_outputs=10)
see.run(tf.global_variables_initializer())
merge = tf.summary.merge_all()
summary =see.run(merge,feed_dict = {xxxx :Xc})
writer.add_summary(summary, i)
...
plt.imshow(Xc, interpolation='nearest')
plt.show()
