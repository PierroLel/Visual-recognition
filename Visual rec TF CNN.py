...
see.run(tf.global_variables_initializer())
merge = tf.summary.merge_all()
writer = tf.summary.FileWriter("C:/Users/woillp/.spyder-py3/tmp/log", see.graph)
summary = see.run(merge,feed_dict={x: np.array(X_train1).reshape(-1,224*224*3),
                                    y_:np.array(y_train1).reshape(-1,4)})
writer.add_summary(summary, i)
writer.close()
...
X_train1, X_test1, y_train1, y_test1 = train_test_split(X_train_cv, y_train_cv ,
                                                                train_size = 32,       # bootstrap size
                                                                stratify = np.zeros(shape=y_train_cv.shape)+1/4)
see.run(train_step,feed_dict={x: np.array(X_train1).reshape(-1,224*224*3),
                                       y_: np.array(y_train1).reshape(-1,4),
                                       keep_prob: 0.8})
print((see.run(W_fc1,feed_dict={x: np.array(X_train1).reshape(-1,224*224*3),
                                                         y_:np.array(y_train1).reshape(-1,4),
                                                         keep_prob: 1})))
W_fc1a,cross_entropya,W_conv6_3a, h_pool1a,W_conv1_3a=see.run([W_fc1,cross_entropy,W_conv6_3,h_pool1,W_conv1_3],
...
see=tf.InteractiveSession()
see.run(tf.global_variables_initializer())
print(int(see.run(cross_entropy,feed_dict={x: np.array(X_train1).reshape(-1,224*224*3),
                                            y_: np.array(y_train1).reshape(-1,4),
                                            keep_prob: 1})))
...
with tf.Session() as se1:
    ...
    for i in range(2):
        ...
        #X_train1, X_test1, y_train1, y_test1 = train_test_split(X_train, y_train , test_size = 1-20/X_train.shape[0]) # bootstrap size
        ...
        #train_step.run(feed_dict={x: np.array(X_train1).reshape(-1,75000)
        #                          ,y_: np.array(y_train1).reshape(-1,4),keep_prob: 0.5})
        toc = time.time()

        #ac_te.append([test_accuracy]) ; ac_tr.append([train_accuracy])
        if i%25 == 0 or i == 100:
            train_accuracy = accuracy.eval(feed_dict={x: np.array(X_train1).reshape(-1,75000)
            ,y_: np.array(y_train1).reshape(-1,4),keep_prob: 1})
            #test_accuracy = accuracy.eval(feed_dict={x: np.array(X_test1).reshape(-1,75000)
            #,y_: np.array(y_test1).reshape(-1,4),keep_prob: 1})
            print('step %d, training accuracy %g' % (i, train_accuracy))
